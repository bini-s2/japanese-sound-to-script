...
- 진입 시 검수 완료된 전체 제목 풀에서 문제를 무작위로 구성합니다.
- 같은 세션에서는 동일 제목을 다시 내지 않습니다.
- 사용자가 아직 풀지 않은 제목을 우선합니다.
- 한 세션에서 같은 아티스트나 동일 문자 유형이 연속으로 과도하게 나오지 않도록 분산합니다.
- 모든 제목을 본 이후에는 전체 풀을 다시 섞습니다.
- 새로 검수된 제목은 랜덤 풀에 즉시 추가할 수 있습니다.
